...
class DaryMorya(Pizza):
    ...
    def gotovka(self,p1):
        ...
        for i in range(101):
            super()._progress_bar(i)
            time.sleep(0.04*p1)  # для задержки
        print(f"\nДобавление соуса: %s"%(self.sause))
        for i in range(101):
            super()._progress_bar(i)
            time.sleep(0.04 * p1)  # для задержки
        ...
        for i in range(101):
            super()._progress_bar(i)
            time.sleep(0.04 * p1)  # для задержки (получается что задержка зависит от колиества пиццы)
        print(f"\nВаша пицца %s %d шт. готова. Приятного аппетита!)"%(self.name, p1))
...
class UserInterface:
    ...
    def __delete(self,zakaz):
        ...
        if n=='1': #изменение количества пицц в заказе
            # смотрим на то что выбрал пользователь и предлагаем изменить количество пицц
            p1 = zakaz.count("Пепперони")
            p2 = zakaz.count("Барбекю")
            p3 = zakaz.count("Дары Моря")
            if p1!=0:
            ...
            #удаляет какую-то выбранную пиццу независимо от количества
            p1 = zakaz.count("Пепперони")
            p2 = zakaz.count("Барбекю")
            p3 = zakaz.count("Дары Моря")
            if p1 != 0:
    ...
    def zakaz_change_choice(self): #когда пользователь выбирает между 1 и 0 вызывается данная функия
        '''выбор между 1 или 0'''
    ...
    def __gotovka(self,zakaz):
        ...
        p1 = zakaz.count("Пепперони")
        p2 = zakaz.count("Барбекю")
        p3 = zakaz.count("Дары Моря") #считываем сколько определенных пицц в корзине у пользователя,
                                    # zakaz - это список в котором хранится какие пиццы выбрал пользователь
        if p1!=0:
...
'''добавить какую-нибудь функцию, когда пользователь ввел количество пицц: 0
функция говорит, что корзина пустая и предлагаает вернуться к меню или закончить программу'''
